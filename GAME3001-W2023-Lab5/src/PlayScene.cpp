...
void PlayScene::GUI_Function()
{
	...
	if(ImGui::CollapsingHeader("Tile Map"))
	{
		...
		for(int i = 0 ; i<m_tileMap.length();i++)
		{
			...
			if (m_tileMap[i]=='S')
			{
				ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.360f, 0.721f, 0.360f, 1.0f)); // Boot strap Primary - green
			}
			else if (m_tileMap[i] == 'G')
			{
				ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.850f, 0.325f, 0.309f, 1.0f)); // Boot strap Warning - Red
			}
			else
			{
				ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.007f, 0.458f, 0.847f, 1.0f)); // Boot Strap Primary - blue
			}
...
{
	std::cout << "Total length is: " << m_pPathList.size() - 1 << std::endl;
