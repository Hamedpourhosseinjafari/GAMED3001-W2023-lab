...
void StarShip::LookWhereYoureGoing(const glm::vec2 target_direction)
{
	...
	constexpr float turn_sensitivity = 4.0f;

	if(GetCollisionWhiskers()[0]|| GetCollisionWhiskers()[4])
	{
		target_rotation += GetTurnRate() * turn_sensitivity;

	}else if(GetCollisionWhiskers()[2]|| GetCollisionWhiskers()[1]|| GetCollisionWhiskers()[3])
	{
		target_rotation -= GetTurnRate() * turn_sensitivity;
	}
