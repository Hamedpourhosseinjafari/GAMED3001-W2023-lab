...
void PlayScene::BuildObstaclePool()
{
	...
	while(!inFile.eof())
	{
		...
		inFile >> x >> y >> w >> h; // data from file line by line
...
// check angle to the target so we can still use LOS Distance for path_nodes
	...
	const auto normalized_direction = Util::Normalize(target_direction); // changes direction to a unit vector(magnitude of 1)
